...
def help(message):
	...
	response += '* news: `reddit headlines`, `espn headlines`, `team headlines [3-letter team abbreviation]`\n'
	response += '* sleeper: `waivers`, `waiver report`, `waiver positions`, `who owns [first] [last]`, `who has [first] [last]`\n'
	response += '* `NFL scores` or `NFL scoreboard` - provides the current week\'s NFL scoreboard\n'
	response += '* `season stats [team abbreviation]` - provides season stats for the identified team\n'
	#response += '* `fantasy scoreboard` - provides the current week\'s fantasy scoreboard\n'
	#response += '* `fantasy schedule [owner name]` - provides the specified owner\'s fantasy schedule\n'
	#response += '* `fantasy rankings` - provides the latest ESPN fantasy power rankings\n'
	#response += '* `margin of victory` - calculates the average margin of victory for each fanatsy team\n'
